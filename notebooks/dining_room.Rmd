...
plot_df = users# %>%
  ...
  # mutate(suggestion_type = fct_drop(suggestion_type))
# contrasts(plot_df$suggestion_type) = contr.treatment(4)
# contrasts(plot_df$has_ax) = contr.treatment
# contrasts(plot_df$has_dx) = contr.treatment
# contrasts(plot_df$has_dxax) = contr.treatment
# contrasts(plot_df$noise_level_f) = contr.treatment

scenario_completed.model = glm(
  scenario_completed ~ (has_ax_only + has_dx_only + has_dxax + noise_level) + has_ax_only:noise_level,
  data=plot_df,
  family = 'binomial'
)
...
scenario_completed.model1 = brm(
  scenario_completed ~ (has_ax_only + has_dx_only + has_dxax + noise_level) + (gender + age_group + robot_experience),
  data=plot_df,
  family = 'bernoulli',
  iter = 4000
)
