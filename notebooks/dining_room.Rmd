...
# Linear Regression...
plot_df = actions
optimal_action.model = lm(
  optimal_ax ~ ((has_dx + has_ax) * noise_level) + (gender + age_group + robot_experience) + start_state + (1 | X1),
  data=plot_df
)
...
optimal_action.model = glmer(
  optimal_ax ~ ((has_dx + has_ax) * noise_level) + (gender + age_group + robot_experience) + (gripper_empty:mislocalized:bowl_visible) + (1 | X1),
  data=plot_df,
  family = 'binomial'
)
summary(optimal_action.model)
