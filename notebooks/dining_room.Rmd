...
```{r, eval=F}
plot(users$scenario_completed)
plotdist(users$scenario_completed, )
```
...
```{r}
plot(sort(users$num_actions_diff))
```
...
actions_diff.model = glm.nb(
  num_actions_diff ~ ((has_dx + has_ax) * noise_level) + (gender + age_group + robot_experience) + num_optimal + scenario_completed_pred,
  data=plot_df,
)
...
```{r, eval=F}
fnorm = fitdist(plot_df$optimal_ax, "norm")
flogis = fitdist(plot_df$optimal_ax, "logis")
fpois = fitdist(plot_df$optimal_ax, "pois")
fnbinom = fitdist(plot_df$optimal_ax, "nbinom")
fgeom = fitdist(plot_df$optimal_ax, "geom")

par(mfrow = c(4, 4))
plot(sort(actions$optimal_ax))
plotdist(actions$optimal_ax, histo=T, demp=T)
descdist(actions$optimal_ax, boot=1000)
...
```{r}
# MCMC estimation
plot_df$user_id = actions %>% group_indices(user_id)
