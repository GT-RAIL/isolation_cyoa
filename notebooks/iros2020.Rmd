...
Options for running the code here:
...
loadCSV = function(filename, age_group_fill) {
  dat = read_csv(
  ...
  contrasts(dat$age_group) = contr.deviation(length(levels(dat$age_group)))
  contrasts(dat$robot_experience) = contr.deviation(length(levels(dat$robot_experience)))
  contrasts(dat$gender) = contr.deviation(length(levels(dat$gender)))

  contrasts(dat$has_ax) = contr.deviation(2)
  contrasts(dat$has_dx) = contr.deviation(2)
  contrasts(dat$has_noise) = contr.deviation(2)
  contrasts(dat$has_dxax) = contr.deviation(2)

  contrasts(dat$noise_level_f) = contr.deviation(length(levels(dat$noise_level_f)))
...
users = loadCSV("users.csv", 3)
actions = loadCSV("actions.csv", 3)
...
Given the nature of the response variable, the family of distributions is assumed to be Bernoulli.
...
scenario_completed.model.v = brm(
  scenario_completed ~ ((has_dx + has_ax) * noise_level_f) + (gender + age_group + robot_experience) + num_optimal,
  family = "bernoulli",
  data = plot_df,
  seed = default_seed,
  save_all_pars = T
...
print(scenario_completed.model.t)
print(scenario_completed.model.t$criteria$loo$estimates)

print(scenario_completed.model.v)
...
noise_levels_hyp_to_test = c(
  "noise_level_f2 = 0", "noise_level_f3 = 0", "Intercept",
  "has_axTRUE:noise_level_f2 = 0", "has_axTRUE:noise_level_f3 = 0",
  "has_dxTRUE:noise_level_f2 = 0", "has_dxTRUE:noise_level_f3 = 0"
)

hyp_results = hypothesis(scenario_completed.model.t, hypothesis = c(hyp_to_test, noise_trend_hyp_to_test))
print(as_tibble(hyp_results$hypothesis))
mcmc_intervals(as.matrix(scenario_completed.model.t), regex_pars = "b_", prob = 0.90, prob_outer = 0.95)

hyp_results = hypothesis(scenario_completed.model.v, hypothesis = c(hyp_to_test, noise_levels_hyp_to_test))
print(as_tibble(hyp_results$hypothesis))
