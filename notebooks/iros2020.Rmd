...
p1 = gg_df %>%
  ggplot(aes(has_ax, ax_decision_duration, fill=.fill_column)) +
    geom_boxplot() +
    geom_count() +
    scale_y_log10() +
    facet_grid(rows = vars(noise_level), cols = vars(has_dx), labeller = label_both) +
...
p2 = gg_df %>%
  ggplot(aes(has_dx, ax_decision_duration, fill=.fill_column)) +
    geom_boxplot() +
    geom_count() +
    scale_y_log10() +
    facet_grid(rows = vars(noise_level), cols = vars(has_ax), labeller = label_both) +
...
p3 = gg_df %>%
  ...
  mutate(has_ax = factor(has_ax)) %>%
  mutate(has_ax = fct_rev(has_ax)) %>%
  ggplot(aes(noise_level, ax_decision_duration, fill=.fill_column)) +
    geom_boxplot() +
    geom_count() +
    scale_y_log10() +
    facet_grid(rows = vars(has_ax), cols = vars(has_dx), labeller = label_both) +
...
From the data, it looks like we might be most interested in a 
