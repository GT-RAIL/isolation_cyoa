...
1. [Indices of Effect Existence and Significance in the Bayesian Framework](https://www.frontiersin.org/articles/10.3389/fpsyg.2019.02767/full). This one is also associated with [this package](https://easystats.github.io/bayestestR/articles/guidelines.html), which has useful links on reporting, presenting data, etc.
...
p1 = 
  preds_p_df %>%
    ...
    ggplot(aes(x = Estimate, y = scenario_completed, color = scenario_completed)) +
      geom_point() +
      geom_vline(xintercept = inv_logit_scaled(2.28)) + # Intercept estimate in the model
      geom_text(aes(x = inv_logit_scaled(2.28)-0.01, y = 0.5, label = "Intercept", color = 0, angle=90)) +
...
h_df = as_tibble(insight::get_parameters(scenario_completed.model.v)) %>%
  transmute(
    ...
    # has_ax_noise_level_1_test = b_Intercept - b_has_ax1 - b_noise_level_f1 + b_has_ax1.noise_level_f1,
    # has_ax_noise_level_2_test = b_Intercept - b_has_ax1 - b_noise_level_f2 + b_has_ax1.noise_level_f2,
...
1. The noise level has a quadratic relationship to the probability of scenario completed with a positive effect size with probability of 0.99 (Median = 1.37, 89% CI [0.63, 2.24]) and can be considered significant (0.0% in ROPE).
1. The effect of no noise (100% accurate suggestions or no suggestions at all) has a probability 1.0 of a positive effect on the probability of completing the scenario (Median = 3.07, 89% CI [1.51, 4.52]) and can be considered significant (0.0% in ROPE).
1. The effect of 20% noise (80% accurate suggestions) has a probability 0.99 of a positive effect on the probability of completing the scenario (Median = 2.63, 89% CI [0.91, 4.39]) and can be considered significant (0.0% in ROPE).
...
# p_df %>%
#   ggplot(aes(noise_level, .value, group = .sample)) +
#     geom_line() +
#     ylim(0, 1)
...
**The number of non-optimal actions that a person took; given their scenario**
