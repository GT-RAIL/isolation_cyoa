...
(ROPE is defined as [-0.055, 0.055], the range is suggested by the literature for logistic models corresponds to a probability range of 0.11. Therefore, a probability change of less than 0.055 is considered no different from a probability change of 0)
...
p1 = 
  preds_p_df %>%
    ...
    ggplot(aes(x = Estimate, y = num_actions_diff)) +
      geom_point() +
      geom_abline(slope = 1, intercept = 0) +
      # geom_text(aes(x = inv_logit_scaled(2.28)-0.01, y = 0.5, lasbel = "Intercept", color = 0, angle=90)) +
      labs(x = "Predicted values") +
      ggtitle("Trends Model")
...
(ROPE is defined as [-0.55, .55] and the range corresponds to a number of actions of 1.1. Therefore, a change of less than 0.55 actions in either direction is considered no different from a change of 0 in the number of actions)
...
```{r}
# Print the parameters, and some initial diagnostics
print(tidy_stan(optimal_ax.model.t))
print(performance::r2(optimal_ax.model.t))

print(tidy_stan(optimal_ax.model.v))
