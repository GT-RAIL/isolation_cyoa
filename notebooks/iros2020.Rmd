...
fits_p_df = scenario_completed.model.t %>% fitted() %>% as_tibble()
...
p1 =
  ...
  ggplot(aes(x = Estimate, y = scenario_completed)) +
    geom_point() +
...
p3 =
  ...
  add_column(predicted = preds_p_df$Estimate,
             scenario_completed = scenario_completed.model.t$data$scenario_completed) %>%
  ggplot(aes(x = fitted, y = predicted, color = scenario_completed)) +
    geom_point() +
    ylim(0, 1) +
    xlim(0, 1) +
    ggtitle("Trends Model")
...
# Plot predictions vs original
fits_p_df = scenario_completed.model.v %>% fitted() %>% as_tibble()
...
p3 =
  ...
  rename(fitted = Estimate) %>%
  add_column(predicted = preds_p_df$Estimate,
             scenario_completed = scenario_completed.model.v$data$scenario_completed) %>%
  ggplot(aes(x = fitted, y = predicted, color = scenario_completed)) +
    geom_point() +
    ylim(0, 1) +
    xlim(0, 1) +
...
1. The presence of action suggesitons has a significant effect on whether a participant completes or not
1. There is a possibly significant quadratic relationship of noise to the probability of a participant completing
1. A noise > 0.0 has a significant effect on the chance of completing compared to when there is no noise
...
plot_df %>%
  ...
  stat_halfeyeh(aes(x = .value), position = position_nudge(y = 0.1), data = fits_p_df) +
  stat_intervalh(aes(x = .prediction), data = preds_p_df)
