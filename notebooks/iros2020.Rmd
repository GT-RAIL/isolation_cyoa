...
plot_summs(actions_diff.model.augment.sparse, plot.distributions = T)

# Plot the DX and noise fit
plot_df$fitted = actions_diff.model.augment.sparse$fitted.values
plot_df$scenario_completed = factor(plot_df$scenario_completed)
...
fits = g %>% add_fitted_draws(optimal_action.model.sparse, n=1)
preds = g %>% add_predicted_draws(optimal_action.model.sparse, n=1)

subset(plot_df, age_group == 0 | age_group == 8) %>%
  ggplot(aes(y = has_ax, x = optimal_ax)) +
  stat_halfeyeh(aes(x = .value), relative_scale = 0.7, position = position_nudge(y = 0.1), data = fits) +
  stat_intervalh(aes(x = .prediction), data = preds) +
  geom_point(data = subset(plot_df, age_group == 0 | age_group == 8))

subset(plot_df, age_group == 0 | age_group == 8) %>%
  ggplot(aes(y = robot_experience, x = optimal_ax)) +
  stat_halfeyeh(aes(x = .value), relative_scale = 0.7, position = position_nudge(y = 0.1), data = fits) +
  stat_intervalh(aes(x = .prediction), data = preds) +
  geom_point(data = subset(plot_df, age_group == 0 | age_group == 8))

subset(plot_df, age_group == 0 | age_group == 8) %>%
  ggplot(aes(y = age_group, x = optimal_ax)) +
  stat_halfeyeh(aes(x = .value), relative_scale = 0.7, position = position_nudge(y = 0.1), data = fits) +
  stat_intervalh(aes(x = .prediction), data = preds) +
  geom_point(data = subset(plot_df, age_group == 0 | age_group == 8))

# ggplot() +
#   conditional_effects(optimal_action.model, effects = "has_ax", re_formula = NULL) +
#   conditional_effects(optimal_action.model, effects = "has_dx", re_formula = NULL) +
#   conditional_effects(optimal_action.model, effects = "noise_level", re_formula = NULL) +
#   facet_grid(rows = 1, cols = 3, scales = "free_x")
```
