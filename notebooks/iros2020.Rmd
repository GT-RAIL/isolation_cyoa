...
preds_p_df = plot_df %>%
  ...
  group_by(num_actions_diff, .prediction) %>%
  count()
p2 =
  preds_p_df %>%
    ggplot(aes(x = num_actions_diff, y = .prediction, fill = n / sum(preds_p_df$n))) +
      geom_tile() +
      geom_text(aes(label = sprintf("%1.2f", n / sum(preds_p_df$n))), size = 10, vjust = 1) +
      scale_fill_gradient(limits = c(0, 1))
...
# Print the parameters, and some initial diagnostics
print(tidy_stan(chose_ax.model.t))
print(performance::r2(chose_ax.model.t))

print(tidy_stan(chose_ax.model.v))
...
# Print the parameters, and some initial diagnostics
print(tidy_stan(optimal_dx.model.t))
print(performance::r2(optimal_dx.model.t))

print(tidy_stan(optimal_dx.model.v))
...
1. The noise level has a linear relationship to the probability of choosing the right diagnoses with a negative effect size (slope) with probability of 0.97 (Median = -0.42, 89% CI [-0.76, -0.09]) and can be considered significant (0.0% in ROPE).
...
p2 = gg_df %>%
  filter(chose_dx == "follow") %>%
  ggplot(aes(has_dx, num_follow_dx / num_actions, fill=.fill_column)) +
    geom_boxplot() +
    geom_count() +
    facet_grid(rows = vars(noise_level), labeller = label_both) +
    labs(y = "Fraction followed DX") +
