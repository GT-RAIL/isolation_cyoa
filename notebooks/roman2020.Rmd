...
- $-\beta_{ax_0} = 0$: The main difference in effects from adding action suggestions (only) vs. the mean of all other conditions is negligible (ceterus paribus)
- $-\beta_{dx_0} = 0$: The main difference in effects from adding diagnosis suggestions (only) vs. the mean of all other conditions is neglible (ceterus paribus)
- $-\beta_{dxax_0} = 0$: The main difference in effects from adding both suggestions vs. the mean of all other conditions is neglible (ceterus paribus)
- $\beta_{noise_L} = 0; \beta_{noise_Q} = 0$: Noise does not have a linear (L) / quadratic (Q) effect on the outcome.

We do not include or test interaction effects in our model because interaction effects add more parameters to our model than can be estimated given the experimental design. Therefore, we assume that noise and the suggestion type have independent effects on each of the outcomes.
...
# TODO: parameters::model_parameters(scenario_completed.model.t, centrality = "median", ci = 0.89, ci_method = "hdi", test = c("pd", "rope"), rope_range = c(-0.055, 0.055), rope_ci = 1, effects = "fixed")
```
...
p1 = pp_check(scenario_completed.model.t, type = "bars_grouped", group = "has_ax_only", stat = "median") + ggtitle("t:has_ax")
p2 = pp_check(scenario_completed.model.t, type = "bars_grouped", group = "has_dx_only", stat = "median") + ggtitle("t:has_dx")
p3 = pp_check(scenario_completed.model.t, type = "bars_grouped", group = "has_dxax", stat = "median") + ggtitle("t:has_dxax")
p4 = pp_check(scenario_completed.model.t, type = "bars_grouped", group = "noise_level", stat = "median") + ggtitle("t:noise_level")
grid.arrange(p1, p2, p3, p4, p0, nrow = 3, ncol = 2)
mcmc_intervals(as.matrix(scenario_completed.model.t), regex_pars = "b_", prob = 0.90, prob_outer = 0.95) +
  ggtitle("Trends Model")

p1 = pp_check(scenario_completed.model.v, type = "bars_grouped", group = "has_ax_only", stat = "median") + ggtitle("v:has_ax")
p2 = pp_check(scenario_completed.model.v, type = "bars_grouped", group = "has_dx_only", stat = "median") + ggtitle("v:has_dx")
p3 = pp_check(scenario_completed.model.v, type = "bars_grouped", group = "has_dxax", stat = "median") + ggtitle("v:has_dxax")
p4 = pp_check(scenario_completed.model.v, type = "bars_grouped", group = "noise_level_f", stat = "median") + ggtitle("v:noise_level")
grid.arrange(p1, p2, p3, p4, p0, nrow = 3, ncol = 2)
...
h_df = as_tibble(insight::get_parameters(scenario_completed.model.t)) %>%
  transmute(
    ...
    has_ax_test = -b_has_ax_only1,
    has_dx_test = -b_has_dx_only1,
    has_dxax_test = -b_has_dxax1,
...
1. AX suggestions
    - Positive effect pd = 97.7%
    - Median = 0.94, 89% CI [0.16, 1.67]
    - Small, Std.Median = 0.48
    - 1.13% in ROPE
1. DXAX suggestions
    - Positive effect pd = 95.8%
    - Median = 0.78, 89% CI [0.06, 1.48]
    - Small, Std.Median = 0.44
    - 2.38% in ROPE
1. Noise Level Quadratic
    - Positive (convex-shape) pd = 96.9%
    - Median = 0.74, 89% CI [0.13, 1.41]
    - Small, Std.Median = 0.41
    - 2.0% in ROPE
...
fits_p_df = p_df %>% add_fitted_draws(scenario_completed.model.v)  # Linear predicted values
...
# If centering ROPE on the posterior distributions
# preds_p_df = p_df %>%
#   add_predicted_draws(scenario_completed.model.v)  %>% # 0, 1 predictions for each predicted value
#   ungroup() %>%
#   count(has_ax_only, has_dx_only, has_dxax, noise_level_f, .prediction)
# rope_value = median(
#   preds_p_df %>%
#     filter(has_ax_only == F) %>%
#     spread(.prediction, n) %>%
#     mutate(.value = `1` / (`0` + `1`)) %>%
#     pull(.value)
# )
...
gg_df %>%
  ...
  ggplot(aes(x = has_ax_only, y = n / 20)) +
    ...
    scale_color_metro() +
    ylim(0, 1) +
    labs(y = "scenario_completed", color = "AX:DX:DXAX:Noise")
...
# rm(preds_p_df) # If using the predictions based ROPE
...
p_df = gg_df %>%
  mutate(estimate = if_else(has_ax_only == "No AX", inv_logit_scaled(eff["Intercept", "Estimate"] + eff["has_ax_only1","Estimate"]), estimate)) %>%
  mutate(estimate = if_else(has_ax_only == "AX", inv_logit_scaled(eff["Intercept", "Estimate"] - eff["has_ax_only1", "Estimate"]), estimate))
p1 = p_df %>%
  filter(scenario_completed == 'Resolved') %>%
  ggplot(aes(has_ax_only, n/20, group = has_ax_only, colour = has_ax_only)) +
...
p_df = gg_df %>%
  mutate(estimate = if_else(has_dxax == "No DXAX", inv_logit_scaled(eff["Intercept", "Estimate"] + eff["has_dxax1","Estimate"]), estimate)) %>%
  mutate(estimate = if_else(has_dxax == "DXAX", inv_logit_scaled(eff["Intercept", "Estimate"] - eff["has_dxax1", "Estimate"]), estimate))
p2 = p_df %>%
  filter(scenario_completed == 'Resolved') %>%
  ggplot(aes(has_dxax, n/20, group = has_dxax, colour = has_dxax)) +
    geom_count(aes(colour = NULL)) +
    geom_boxplot(aes(y = estimate)) +
    geom_signif(y_position = 1.05, xmin = 1, xmax = 2, annotation = "*", textsize = 8, color = "black") +
    scale_y_continuous(limits = c(0.5, 1.1)) +
...
$$\begin{aligned}
logit^{-1}(p_{ij}) &= \beta_0 + \beta_{ax}\text{ax_only}_i + \beta_{dx}\text{dx_only}_i + \beta_{dxax}\text{dxax}_i +\\
...
optimal_ax.model.t = brm(
  optimal_ax ~ 0 + Intercept + (has_ax_only + has_dx_only + has_dxax + noise_level) + (age_group + robot_experience + gender) + num_optimal + state_idx_rescaled + (1 | user_id),
  family = bernoulli,
  prior = c(set_prior("normal(0, 10)", class = "b"),
            set_prior("student_t(3, 0, 10)", class = "sd")),
  data = plot_df,
...
optimal_ax.model.v = brm(
  optimal_ax ~ 0 + Intercept + (has_ax_only + has_dx_only + has_dxax + noise_level_f) + (age_group + robot_experience + gender) + num_optimal + state_idx_rescaled + (1 | user_id),
  family = bernoulli,
  prior = c(set_prior("normal(0, 10)", class = "b"),
            set_prior("student_t(3, 0, 10)", class = "sd")),
  data = plot_df,
...
p1 = pp_check(optimal_ax.model.t, type = "bars_grouped", group = "has_ax_only", stat = "median") + ggtitle("t:has_ax")
p2 = pp_check(optimal_ax.model.t, type = "bars_grouped", group = "has_dx_only", stat = "median") + ggtitle("t:has_dx")
p3 = pp_check(optimal_ax.model.t, type = "bars_grouped", group = "has_dxax", stat = "median") + ggtitle("t:has_dxax")
p4 = pp_check(optimal_ax.model.t, type = "bars_grouped", group = "noise_level", stat = "median") + ggtitle("t:noise_level")
grid.arrange(p1, p2, p3, p4, p0, nrow = 3, ncol = 2)
mcmc_intervals(as.matrix(optimal_ax.model.t), regex_pars = "b_|^shape", prob = 0.90, prob_outer = 0.95) +
  ggtitle("Trends Model")

p1 = pp_check(optimal_ax.model.v, type = "bars_grouped", group = "has_ax_only", stat = "median") + ggtitle("v:has_ax")
p2 = pp_check(optimal_ax.model.v, type = "bars_grouped", group = "has_dx_only", stat = "median") + ggtitle("v:has_dx")
p3 = pp_check(optimal_ax.model.v, type = "bars_grouped", group = "has_dxax", stat = "median") + ggtitle("v:has_dxax")
...
1. AX suggestions
    - Positive effect pd = 96.4%
    - Median = 0.5, 89% CI [0.06, 0.95]
    - Small, Std.Median = 0.28
    - 3.43% in ROPE (n.s.)
1. DXAX suggestions
    - Positive effect pd = 98.3%
    - Median = 0.58, 89% CI [0.15, 1.01]
    - Small, Std.Median = 0.28
    - 1.85% in ROPE
...
p_df =
  ...
  data_grid(optimal_ax,
            ...
            age_group,
            robot_experience,
            num_optimal = seq_range(num_optimal, n = 2),
            state_idx_rescaled = seq_range(state_idx_rescaled, n = 3),
...
gg_df %>%
  ...
  ggplot(aes(x = has_ax_only, y = num_optimal_ax / num_actions)) +
    ...
    scale_color_metro() +
    ylim(0, 1) +
    labs(y = "correct_ax", color = "AX:DX:DXAX:Noise")
...
gg_df = plot_df %>%
  ...
  mutate(scenario_completed = fct_recode(scenario_completed, Unresolved="0", Resolved = "1"),
         ...
         has_dxax = fct_recode(has_dxax, "No DXAX"="FALSE", "DXAX"="TRUE"),
         noise_level = fct_recode(noise_level, "Acc: 100%"="0.0", "Acc: 90%"="1.0", "Acc: 80%"="2.0")) %>%
  count(user_id, study_condition, optimal_ax, num_actions, has_ax_only, has_dx_only, has_dxax, noise_level,
        name = "num_optimal_ax") %>%
  mutate(estimate = -1)
...
p1 = p_df %>%
  filter(optimal_ax == "1") %>%
  ggplot(aes(has_ax_only, num_optimal_ax / num_actions, group = has_ax_only, colour=has_ax_only)) +
    geom_count(aes(colour = NULL)) +
    geom_boxplot(aes(y = estimate)) +
    # geom_signif(y_position = 1.07, xmin = 1, xmax = 2, annotation = "n.s.", textsize = 8, color = "black") +
...
p2 = p_df %>%
  filter(optimal_ax == "1") %>%
  ggplot(aes(has_dxax, num_optimal_ax / num_actions, group = has_dxax, colour=has_dxax)) +
    geom_count(aes(colour = NULL)) +
    geom_boxplot(aes(y = estimate)) +
...
optimal_dx.model.t = brm(
  optimal_dx ~ 0 + Intercept + (has_ax_only + has_dx_only + has_dxax + noise_level) + (age_group + robot_experience + gender) + num_optimal + state_idx_rescaled + (1 | user_id),
  family = bernoulli,
  prior = c(set_prior("normal(0, 10)", class = "b"),
            set_prior("student_t(3, 0, 10)", class = "sd")),
  data = plot_df,
...
optimal_dx.model.v = brm(
  optimal_dx ~ 0 + Intercept + (has_ax_only + has_dx_only + has_dxax + noise_level_f) + (age_group + robot_experience + gender) + num_optimal + state_idx_rescaled + (1 | user_id),
  family = bernoulli,
  prior = c(set_prior("normal(0, 10)", class = "b"),
            set_prior("student_t(3, 0, 10)", class = "sd")),
  data = plot_df,
...
p1 = pp_check(optimal_dx.model.t, type = "bars_grouped", group = "has_ax_only", stat = "median") + ggtitle("t:has_ax")
p2 = pp_check(optimal_dx.model.t, type = "bars_grouped", group = "has_dx_only", stat = "median") + ggtitle("t:has_dx")
p3 = pp_check(optimal_dx.model.t, type = "bars_grouped", group = "has_dxax", stat = "median") + ggtitle("t:has_dxax")
p4 = pp_check(optimal_dx.model.t, type = "bars_grouped", group = "noise_level", stat = "median") + ggtitle("t:noise_level")
grid.arrange(p1, p2, p3, p4, p0, ncol = 2, nrow = 3)
mcmc_intervals(as.matrix(optimal_dx.model.t), regex_pars = "b_|^shape", prob = 0.90, prob_outer = 0.95) +
  ggtitle("Trends Model")

p1 = pp_check(optimal_dx.model.v, type = "bars_grouped", group = "has_ax_only", stat = "median") + ggtitle("v:has_ax")
p2 = pp_check(optimal_dx.model.v, type = "bars_grouped", group = "has_dx_only", stat = "median") + ggtitle("v:has_dx")
p3 = pp_check(optimal_dx.model.v, type = "bars_grouped", group = "has_dxax", stat = "median") + ggtitle("v:has_dxax")
p4 = pp_check(optimal_dx.model.v, type = "bars_grouped", group = "noise_level_f", stat = "median") + ggtitle("v:noise_level")
grid.arrange(p1, p2, p3, p4, p0, ncol = 2, nrow = 3)
...
1. DX suggestions
    - Positive effect pd = 96.9%
    - Median = 0.41, 89% CI [0.06, 0.74]
    - Small, Std.Median = 0.22
    - 3.33% in ROPE (n.s.)
1. DXAX suggestions
    - Positive effect pd = 98.4%
    - Median = 0.45, 89% CI [0.11, 0.79]
    - Small, Std.Median = 0.21
    - 2.17% in ROPE
1. Noise Level Linear
    - Negative (negative slope) pd = 99.4%
    - Median = -0.47, 89% CI [-0.81, -0.15]
    - Small, Std.Median = 0.20
    - 1.40% in ROPE
...
chose_ax.model.t = brm(
  chose_ax ~ 0 + Intercept + (has_dxax + noise_level) + (age_group + robot_experience + gender) + num_optimal + state_idx_rescaled + (1 | user_id),
  family = bernoulli,
  prior = c(set_prior("normal(0, 10)", class = "b"),
            set_prior("student_t(3, 0, 10)", class = "sd")),
  data = plot_df,
...
chose_ax.model.v = brm(
  chose_ax ~ 0 + Intercept + (has_dxax + noise_level_f) + (age_group + robot_experience + gender) + num_optimal + state_idx_rescaled + (1 | user_id),
  family = bernoulli,
  prior = c(set_prior("normal(0, 10)", class = "b"),
            set_prior("student_t(3, 0, 10)", class = "sd")),
  data = plot_df,
...
# Include posterior predictive checks of the sampling (are sampling according
# to the data that we actually have?)
p2 = pp_check(chose_ax.model.t, type = "bars_grouped", group = "has_dxax", stat = "median") + ggtitle("t:has_dx")
p3 = pp_check(chose_ax.model.t, type = "bars_grouped", group = "noise_level", stat = "median") + ggtitle("t:noise_level")
grid.arrange(p2, p3, ncol = 2)
mcmc_intervals(as.matrix(chose_ax.model.t), regex_pars = "b_|^shape", prob = 0.90, prob_outer = 0.95) +
  ggtitle("Trends Model")

p2 = pp_check(chose_ax.model.v, type = "bars_grouped", group = "has_dxax", stat = "median") + ggtitle("v:has_dx")
p3 = pp_check(chose_ax.model.v, type = "bars_grouped", group = "noise_level_f", stat = "median") + ggtitle("v:noise_level")
...
1. Noise Level Linear
    - Negative (negative slope) pd = 95.6%
    - Median = -0.39, 89% CI [-0.75, -0.01]
...
chose_dx.model.t = brm(
  chose_dx ~ 0 + Intercept + (has_dxax + noise_level) + (age_group + robot_experience + gender) + num_optimal + state_idx_rescaled + (1 | user_id),
  family = bernoulli,
  prior = c(set_prior("normal(0, 10)", class = "b"),
            set_prior("student_t(3, 0, 10)", class = "sd")),
  data = plot_df,
...
chose_dx.model.v = brm(
  chose_dx ~ 0 + Intercept + (has_dxax + noise_level_f) + (age_group + robot_experience + gender) + num_optimal + state_idx_rescaled + (1 | user_id),
  family = bernoulli,
  prior = c(set_prior("normal(0, 10)", class = "b"),
            set_prior("student_t(3, 0, 10)", class = "sd")),
  data = plot_df,
...
# Include posterior predictive checks of the sampling (are sampling according
# to the data that we actually have?)
p1 = pp_check(chose_dx.model.t, type = "bars_grouped", group = "has_dxax", stat = "median") + ggtitle("t:has_ax")
p3 = pp_check(chose_dx.model.t, type = "bars_grouped", group = "noise_level", stat = "median") + ggtitle("t:noise_level")
grid.arrange(p1, p3, ncol = 2)
mcmc_intervals(as.matrix(chose_dx.model.t), regex_pars = "b_|^shape", prob = 0.90, prob_outer = 0.95) +
  ggtitle("Trends Model")

p1 = pp_check(chose_dx.model.v, type = "bars_grouped", group = "has_dxax", stat = "median") + ggtitle("v:has_ax")
p3 = pp_check(chose_dx.model.v, type = "bars_grouped", group = "noise_level_f", stat = "median") + ggtitle("v:noise_level")
