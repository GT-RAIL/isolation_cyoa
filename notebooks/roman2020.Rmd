...
$$FRR_i = Bernoulli(p_i)$$
$$\begin{aligned}
logit^{-1}(p_i) &= \beta_0 + \beta_{ax}\text{ax_only}_i + \beta_{dx}\text{dx_only}_i + \beta_{dxax}\text{dxax}_i +\\ &\beta_{noise_L}\text{noise}_i + \beta_{noise_Q}\text{noise}_i +\\
...
gg_df =
  plot_df %>%
    ...
    mutate(user_id = factor(user_id)) %>%
    mutate(optimal_ax = fct_recode(optimal_ax, incorrect="0", correct = "1")) %>%
    count(suggestion_type, noise_level, optimal_ax, num_actions,
          name = "num_optimal_ax")
...
1. DX suggestions
    ...
    - Median = 0.54, 89% CI [0.12, 0.94]
    - Small, Std.Median = 0.26
    - 1.98% in ROPE
1. Noise Level Linear
    - Negative (negative slope) pd = 97.9%
    - Median = -0.42, 89% CI [-0.78, -0.10]
