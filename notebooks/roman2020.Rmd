...
# # Plot by the DX, AX, Noise factors (original analyses)
# gg_df = plot_df %>%
#   mutate(scenario_completed = factor(scenario_completed)) %>%
#   mutate(scenario_completed = fct_recode(scenario_completed, incomplete="0", complete = "1")) %>%
#   mutate(noise_level = fct_rev(noise_level)) %>%
...
$$frr_i = Bernoulli(p_i)$$
$$\begin{aligned}
logit^{-1}(p_i) &= \beta_0 + \beta_{ax}\text{ax_only}_i + \beta_{dx}\text{dx_only}_i + \beta_{dxax}\text{dxax}_i + \beta_{noise}\text{noise}_i +\\
...
1. AX suggestions
    ...
    - Median = 2.24, 89% CI [1.30, 3.18]
    - Medium, Std.Median = 0.58
    - 0.0% in ROPE
